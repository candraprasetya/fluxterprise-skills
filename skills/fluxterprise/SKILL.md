---
name: fluxterprise
description: "Quality gate system for AI coding agents. Enforce craft standards before any UI ships."
allowed-tools: Read Write Edit Glob Grep
---
# fluxterprise

> Quality Gate System for AI Coding Agents
>
> Created by **Candra Prasetya**

> Follow these standards whenever generating or building UI for a website, web app, or any interface.
> The goal: every deliverable passes a **quality gate** before it ships. Not filtering bad output, but enforcing good craft.
> Before any UI work, ask the user (in their chat language) when fluxterprise applies: during the work, or after it is done. Do not start until they answer.

---

## First-Run Install Wizard

fluxterprise is a system: this core file plus optional **skills**, one per concern (`fluxterprise-ui`, `fluxterprise-copywriting`, and so on, each a folder with a `SKILL.md`). This section installs them once. It is an **offer, not a requirement**: if the user prefers, this file alone remains a complete gate.

**Skip this section** if the project's entry file (`AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, or the equivalent the running tool reads at session start) already contains a fluxterprise pointer block. That means fluxterprise is already installed. Come back only when the user asks to add or remove a skill.

If no fluxterprise pointer exists and this file is being read for the first time, run the wizard:
...
1. **Declare the setup before doing anything.** Tell the user you will (a) get the chosen skill(s) in place in `skills/<name>/` subfolders next to this file (the user fetches them; the agent never downloads from the network, see step 4), and (b) append a fluxterprise pointer block at the end of the project's entry file. Get approval. Never modify the entry file silently.
2. **Ask which skills to install** (multi-select, in the user's chat language). List only the skills that exist in this version of fluxterprise:
   - **1. All** (recommended): install every available skill. Choose this when the work spans UI, copy, people, or mobile layout.
   - **2. `fluxterprise-ui`** (UI / visual): pick this for building or editing a website, web app, or interface: color, layout, components, decoration, motion.
   - **3. `fluxterprise-copywriting`** (copy & text): pick this for writing or editing copy: headlines, CTAs, value propositions, tone, landing-page text, product prose.
   - **4. `fluxterprise-human`** (people): pick this for making sure a UI works for people with different eyes, hands, and setups: contrast, keyboard, focus, states.
   - **5. `fluxterprise-layoutmobile`** (mobile / responsive): pick this for layouts that have to reflow across screen sizes, phone to desktop: breakpoints, scale, grids, overflow, tap targets.
   - **6. `fluxterprise-code`** (code comments): pick this for writing or editing code comments: remove generic AI-slop comments, keep the valuable ones, never touch the code.
   - **7. `fluxterprise-flutter-motion`** (Flutter motion): pick this for 60fps animations, micro-interactions, cross-platform motion consistency.
   - **8. `fluxterprise-testing`** (testing): pick this for unit, widget, integration, accessibility, and visual regression testing.
...
3. **Resolve direction** (only if a UI skill was selected). Check for `DESIGN.md` or explicit brand/style direction. If none exists, be honest that fluxterprise is a **gate, not a beautifier**: without direction the output tends toward monotonous. Recommend having a `DESIGN.md`, then offer these paths:
   ...
   - **2. The agent supplies direction, with an honest warning.** The agent writes the direction itself, stating explicitly that agent-generated style tends toward default AI taste, which the gate is designed to catch, so the result is likely monotonous. If chosen, still ask a minimal brief (product, audience, mood) before building.
   - **3. The user skips direction for now.** Proceed without a `DESIGN.md`. Any UI built this way must be labeled *"draft without direction"* with dials ENERGY 1 / RHYTHM 1 / MOTION 1 (FG-37), and is not a shippable deliverable.
4. **Get the chosen skill(s) in place; the user does the fetching, never the agent.** A `SKILL.md` is instructions the agent will obey, so an agent that downloads one at runtime is fetching its own next prompt: do not do it, and do not ask for network access here. The skills ship as folders in the release (`skills/<name>/SKILL.md`). If a chosen skill folder is missing next to this file, tell the user which ones are missing and that they come with the release matching this core, so a newer skill never mixes with an older one. `fluxterprise-human` also needs `contrast-check.py` from that same folder.
5. **Append the pointer block at the END of the project's entry file** (the file the running tool reads at session start: `CLAUDE.md` for Claude Code, `AGENTS.md` for Codex, `GEMINI.md` for Gemini CLI, and so on). If that file does not exist, create it. Never modify existing content:
   ...
   <!-- fluxterprise:end -->
   ```
   The packaged installers write the same two markers, so whichever install path runs last replaces the block instead of adding a second one. If an older fluxterprise block exists (even without the markers), replace just that block instead of appending a duplicate.
...
fluxterprise is used one of two ways. At the start of a session, ask the user which applies, in the user's chat language (not the language of this file). Do not start UI work until they answer.

> **When do you want to use fluxterprise?**
> 1. **DURING** the project, while working (planning & execution). I will apply the standards while I write, so the quality gate runs from the start.
...
- **Mode 1 (During):** follow the standards while generating. This enforces craft from the start and ends with the Quality Gate. Use it when building new UI.
- **Mode 2 (After):** audit an already-finished project. Produce a numbered findings list in `fluxterprise/audit-001-YYYY-MM-DD.md` (numbers keep rising). Each finding cites the violated rule (FG-XX) and a one-line reason. Priority follows the rule tier: Hard Gate = HIGH, Purpose Gate = MEDIUM, Quality Locks = LOW. Do not modify anything until the user approves specific numbers; numbers not mentioned are not touched. Then fix the approved items and write a follow-up report.

> **Three steps run in both modes, whatever else is driving the session.** Another skill or planning workflow, a brainstorm or a spec, may lead the work, but it never replaces these: settle direction before building (FG-37), ask before creating any asset (FG-23), and run the Quality Gate before delivering. Naming a process is not the same as running it.

## What This Is (and What It Isn't)

`fluxterprise` is a **quality gate**, not a style guide. It enforces craft standards for AI coding agents, ensuring every deliverable meets a bar before it ships.

- This document does **not** impose an aesthetic: no prescribed colors, fonts, layouts, or "house style".
- This document does **not** ban visual techniques (gradients, glassmorphism, badges, card grids). Those are tools. What it requires is **technique with purpose**.
- This document does two things only:
  ...
  2. Holds the result to a **craftsmanship bar**: the output must be alive and specific, not just "clean". See Part 3.

`fluxterprise` is one of three files, and it is a **gate, not a source of direction**:
...
- `AGENTS.md` (or `CLAUDE.md`, `GEMINI.md`, etc.) routes the agent: "for UI work, read `DESIGN.md` for direction, then `fluxterprise` as the gate."
- `fluxterprise` enforces craft and requires intentionality. It does not invent direction; the Design Read (Part 3) turns a brief into dials.
...
Craft is not revealed by removing bad patterns; it must be **built**, not assumed. A sterile result means either direction was missing or craft was not added, and both are failures to fix. The fix is never "add more rules"; it is "state the purpose and raise the bar".

## Core Philosophy

The gate requires technique with purpose, not technique itself. Before using any visual technique, answer: **what does this serve?** If the only answer is "it looks AI" or "it looks safe", the technique must go or be reworked. If the answer names a hierarchy, identity, or readability goal, it stays, and the reason is written down.

## Implementation Enforcement (Mandatory)

These rules apply to ALL projects, not just Flutter.

### No Empty Deliverables

- [ ] **Every file you create must contain real, functional code.** No placeholder files, no `// TODO: implement later`, no empty classes, no `// Coming soon`.
- [ ] **Every folder you create must contain at least one file with real code.** A folder with zero files is a FAIL. A folder with only empty files is a FAIL.
- [ ] **Every feature you build must be complete end-to-end.** Domain → Data → DI → Presentation → Wiring. A feature with only UI and no data layer is a FAIL.

### Build Order (Mandatory)

For ANY new feature in ANY framework:

1. **Define the data model** — what data does this feature work with?
2. **Define the business logic** — what operations does this feature support?
3. **Define the API/data source** — where does the data come from?
4. **Wire the dependency injection** — how are the pieces connected?
5. **Build the UI** — how does the user interact with this feature?
6. **Add error/empty/loading states** — what happens when things go wrong?

**Never jump to step 5 before steps 1-4 are complete.** A UI without data behind it is a mockup, not a feature.

### The "Would It Actually Run?" Test

Before delivering any code, ask:
- If I ran `flutter pub get && flutter run`, would it compile?
- If I tapped every button, would something happen?
- If the API returned an error, would the user see a helpful message?
- If the device was offline, would the app crash or show a graceful state?

If any answer is "no" or "I don't know", the deliverable is not done.
...
2. It has its own identity and character (see Part 3: Craftsmanship Toolkit).
3. It actually works (see The Craftsmanship Standard).

## The Craftsmanship Standard

"Not shipped" is the floor, not the goal. A design passes when it meets five criteria. Use these as questions, not recipes.
...
## Part 1: Quality Warning Signs

These are the most common patterns found in AI-generated designs. Use this table to **audit** your output: scan for clusters, then ask each one "what does this serve?" A single pattern from this list is fine if it serves a purpose, unless a **Hard Gate** rule in Part 2 forbids it (FG-02, FG-03, FG-17, FG-18, FG-23 to FG-28, FG-32 to FG-38). What makes a design fall below the gate is many of these appearing together with no reason. This is a **diagnostic scan, not a ban list**: Part 1 itself bans nothing, but the Hard Gate rules in Part 2 are absolute, and every other pattern must pass the purpose test (Part 2, Purpose Gate group).
...
| **Irrelevant FAQ** | FAQ contains generic template questions with no real relevance to the product |
...
| **Not Keyboard Navigable** | UI can only be used with a mouse. Interactive elements can't be reached with Tab |

---

## Part 2: Mandatory Rules (FG-01 to FG-38, grouped)
...
- **Carve-out**: documentation of this rule is exempt: the numbered section headings in this file (`FG-XX` rules and `C-1` to `C-5` principles), the em dash example in Part 1, the rule's own definition, any Quality Gate item that quotes it, and the `Em Dashes` section in the copywriting skill (`skills/fluxterprise-copywriting/SKILL.md`). These are documentation structure, not UI text.

#### FG-03 — Mobile Responsiveness
...
#### FG-23 — Clarification & Visual Assets
...
**FORBIDDEN**: nav items pointing to sections that do not exist (see FG-24)
...
#### FG-33 — No File/CSS Patching via Scripts
...
#### FG-34 — Every Theme You Ship Must Work
...
#### FG-35 — Verify Before You Deliver
...
- An element passes only when its action really happens: a link navigates somewhere that exists, a form submit shows validation or success, a toggle changes state, a modal opens and closes. Code that only looks interactive is a dead control (see FG-26)
...
- Style direction is the product owner's identity, not a generic pattern; this gate only applies on top of it
- If `DESIGN.md` itself asks for a named pattern that collides with a rule, do not silently follow it and do not silently override it: name the element, name the rule it collides with, and ask the owner to keep it or drop it
- Ask only about a named pattern, never about a stylistic choice the direction is entitled to make: a bold palette or an unusual typeface is identity, not a gate violation
...
#### FG-38 — Real Content or Honest Placeholder
...
- Placeholders are written as what they are: `[REAL DATA]`, "Coming soon", never disguised as final (see FG-23)
...
- A gradient that separates one level of hierarchy from another is craft; the same gradient covering the whole page is a default. The technique is not the problem, the purpose is
...
These are consistency requirements. They stay as-is, with two adjustments: FG-05 now references the RHYTHM dial, and FG-31 is upgraded to the keystone rule.

#### FG-05 — Layout & Page Structure
...
- A theme toggle you ship must work correctly in BOTH modes. A dark mode that breaks the light mode is a defect (see FG-34)
...
#### FG-30 — Do Not Clone Popular Products
...
#### FG-31 — Every Decision Must Have a Reason (Write It Down)
...
## Part 3: Craftsmanship Toolkit

A gate can enforce standards, but it cannot add energy. Removing bad patterns leaves a void, and the model fills that void with its most generic output. Craft must be **added** deliberately. This Part is that mechanism: positive requirements, not bans.
...
**Flutter / Mobile anchors:**

| Dial | 1 (Calm) | 2 (Balanced) | 3 (Bold) |
|---|---|---|---|
| **ENERGY** | LINE, WhatsApp (utility-first, minimal chrome) | Cash App, Grab (brand-forward, clean surfaces) | Revolut, Monzo (bold color, custom shapes, strong identity) |
| **RHYTHM** | Every screen follows same scaffold pattern | Key screens break pattern (detail with SliverAppBar, dashboard with GridView) | Each screen has distinct composition for its content |
| **MOTION** | InkWell splash, AnimatedContainer for state changes | Hero transitions, AnimatedSwitcher for content changes | Coordinated staggered animations, parallax with ScrollController |

Why three levels and not ten: a model and a reviewer can reliably tell "is this section uniform or varied?" (binary, checkable). They cannot reliably judge "is this a 6 or a 7?" (continuous, uncheckable). Three levels make craft enforceable.

Example sets: a designer portfolio sets ENERGY 3, RHYTHM 3, MOTION 2. A public-service site sets ENERGY 1, RHYTHM 1, MOTION 1. A fintech app sets ENERGY 2, RHYTHM 2, MOTION 1.
...
- **One deliberate accent**: one color or gesture used sparingly at the key moment. Zero accents is sterile; an accent everywhere is a default
...
3. **No direction and the user cannot be asked**: label the output *"draft without direction"*, set the honest default dials **ENERGY 1 / RHYTHM 1 / MOTION 1** (see FG-37), and do not present it as a deliverable.
...
Run this gate BEFORE delivering. Output its status with your deliverable as a **PASS/FAIL report**: one line per item, and every `PASS` backed by concrete evidence (e.g. "FG-26 PASS: every button has a real `href` or `onClick`; no dead controls", "FG-35 PASS: ran the build and clicked every control: Signup -> /signup, empty form -> validation, mobile menu -> opens, no console errors").
...
The gate has five blocks: Hard Gate (absolute), Purpose Gate (technique + written reason), Craftsmanship (dials + levers), Craftsmanship & Quality Locks (C-1..C-5 plus the consistency locks FG-05, FG-11, FG-15, FG-16, FG-20, FG-21, FG-29, FG-30, FG-31), and Performance (render timing, bundle size, accessibility audit).

### Block 1: Hard Gate (absolute)

Before declaring the design done, answer every question below. All answers must be **no**:

- [ ] Is there an em dash anywhere in the text, outside the FG-02 carve-out? *(FG-02)*
- [ ] Is there any horizontal overflow, text escaping its container, or broken layout on mobile? *(FG-03)*
- [ ] Are there any statistics without a real source (10K+ Users, 99.9% Uptime, etc.)? *(FG-17)*
- [ ] Are there any fictional testimonials (AI avatars, random names or job titles)? *(FG-18)*
- [ ] Were any visual assets (logo, avatar/profile photo, statistics, testimonials, or navigation structure) created without explicit instructions or confirmation, and without an honest placeholder? *(FG-23)*
- [ ] Are there navbar links pointing to sections or pages that do not exist? *(FG-24)*
- [ ] Is there any text with contrast below the WCAG AA standard (4.5:1 for normal text, 3:1 for large text)? *(FG-25)*
- [ ] Are there any buttons, dropdowns, or forms that do nothing, with no real behavior and no `// TODO` + visible label? *(FG-26)*
- [ ] Does the UI lack an empty state, loading state, or error state? *(FG-27)*
- [ ] Does the FAQ contain generic questions that are not relevant to the product? *(FG-28)*
- [ ] Can the UI not be navigated by keyboard (Tab, Enter, Escape) or is there no visible focus state? *(FG-32)*
- [ ] Was any feature added by patching source/CSS with an external script instead of writing it in source? *(FG-33)*
- [ ] If a theme toggle exists, does one mode (light or dark) break styles, fonts, or layout? *(FG-34)*
- [ ] Was the app delivered without being run or built, or without a recorded click-through of every interactive element? *(FG-35)*
- [ ] Are there any fabricated security, compliance, performance, or customer claims? *(FG-36)*
- [ ] Was the design built without direction and not labeled *"draft without direction"* with honest default dials ENERGY 1 / RHYTHM 1 / MOTION 1? *(FG-37)*
- [ ] Is there any realistically-styled content that was fabricated (testimonials, features, statistics, ghost links, fictional team) without a real source? *(FG-38)*
...
- [ ] Do gradients/glows appear as a default with no stated hierarchy or brand purpose? *(FG-01)*
- [ ] Are there generic icons (sparkle, star, magic, lightning, diamond, robot, orb), an icon set picked for its library look (Lucide-style), or icons irrelevant to their content, with no written relevance? *(FG-04)*
- [ ] Is there a large monospace font, uppercase label with wide tracking, or a typeface chosen without a written brand-character reason? *(FG-06)*
- [ ] Is there a background grid, blueprint, graph paper, or dot pattern without a written visual-identity purpose? *(FG-07)*
- [ ] Are arrows placed on almost every button purely as decoration, with no written purpose? *(FG-08)*
- [ ] Are there capsule badges ("AI Powered", "Beta", "New", "Secure", "Fast") with no real function, the full capsule + thin border + glow + uppercase combination, or a pill parked above the H1 holding a label the headline already says? *(FG-09)*
- [ ] Is glassmorphism applied to more than 1-2 elements simultaneously (navbar + card + modal + sidebar)? *(FG-10)*
- [ ] Is a large shadow applied to every component, with no written elevation reason, making the page feel like it is floating? *(FG-12)*
- [ ] Is glow applied to cards, buttons, badges, icons, backgrounds, and borders simultaneously? *(FG-13)*
- [ ] Do all feature cards have identical size, icon, padding, and layout, with no written hierarchy reason? *(FG-14)*
- [ ] Do all elements use template animations simultaneously (Fade Up + Floating + Scale + Bounce) without a written UX purpose, or does the motion contradict the declared MOTION dial? *(FG-19)*
- [ ] Are there generic illustrations (Undraw, Storyset, 3D blob) with no written product connection? *(FG-22)*

### Block 3: Craftsmanship (required to be alive, not just clean)
...
- [ ] Does the layout follow an AI template (generic Hero+cards, "How It Works" always 3 steps, "Trusted By" logo bar, bento-grid mosaic, fake terminal window, 3 pricing columns, 4-column footer with no variation, uniform section rhythm), or does the section rhythm contradict the declared RHYTHM dial? *(FG-05)*
- [ ] Are all elements (buttons, cards, inputs, badges) made pill-shaped with no radius variation? *(FG-11)*
- [ ] Are CTAs still generic (Get Started, Learn More, Try Now, Explore, Discover)? *(FG-15)*
- [ ] Are there any AI marketing buzzwords (AI Powered, Seamless, Revolutionary, Cutting Edge, etc.)? *(FG-16)*
- [ ] Does the design still feel generic even if the logo and product name are swapped? *(FG-20)*
- [ ] Was dark mode forced as a default without a branding/user reason, or was a required light/dark toggle deferred with an excuse? *(FG-21)*
- [ ] Does the color palette exceed 2-3 core colors + 1 accent without a clear design system? *(FG-29)*
- [ ] Does the overall design look like a clone of another popular product (Linear, Vercel, Stripe, Notion, etc.)? *(FG-30)*
- [ ] Is there any major visual decision (color, layout, typography, spacing, cards, illustration) whose reason cannot be written in one line? *(FG-31)*

### Block 5: Performance (render timing, bundle, accessibility)

All answers must be **no**:

- [ ] **Web**: Does the Largest Contentful Paint (LCP) exceed 2.5s, or First Input Delay (FID) exceed 100ms, or Cumulative Layout Shift (CLS) exceed 0.1? *(Core Web Vitals)*
- [ ] **Web**: Does the Lighthouse performance score fall below 90? *(Performance)*
- [ ] **Web**: Does the total bundle size exceed 200KB gzipped for initial load? *(Bundle size)*
- [ ] **Web**: Are images served without `loading="lazy"` for below-the-fold content? *(Lazy loading)*
- [ ] **Web**: Are images served in WebP/AVIF format where supported? *(Image optimization)*
- [ ] **Flutter**: Does any animation drop below 60fps (16.67ms per frame)? *(Frame timing)*
- [ ] **Flutter**: Does any build method take more than 1ms to complete? *(Build cost)*
- [ ] **Flutter**: Is `MediaQuery.of(context)` used instead of specific accessors (`MediaQuery.sizeOf`, `MediaQuery.paddingOf`)? *(Rebuild scope)*
- [ ] **Flutter**: Are `const` constructors missing on immutable widgets? *(Allocation cost)*
- [ ] **Accessibility**: Does the UI fail any axe-core or Lighthouse accessibility audit? *(A11y automation)*
- [ ] **Accessibility**: Is `prefers-reduced-motion` (web) or `MediaQuery.disableAnimations` (Flutter) not respected? *(Motion sensitivity)*

Evidence required:
- **Web**: Paste Lighthouse score, Core Web Vitals, and gzipped bundle size
- **Flutter**: Paste DevTools frame timing screenshot or `--profile` build output
- **Accessibility**: Paste axe-core or Lighthouse a11y score
